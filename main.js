// 1. DATA CATALOG
// MASTER PRODUCT CATALOG - The one place to rule them all
// 1. MASTER PRODUCT CATALOG
// MASTER PRODUCT CATALOG - Clean version with only your 8 main products
const catalog = {
    "Rigid Boxes": [
        "Magnetic Snap Box", 
        "Lid & Base Box", 
    ...
    "Snap-Lock (1-2-3) Bottom": {
        ...
        link: "products/art-card/snap-lock-bottom.html"
    },
...
// Ensure this runs only after the full page loads
...
// Product canonical links - single source of truth
const productLinks = {
    "Rigid Boxes": "products/rigid-boxes.html",
    "Corrugated": "products/corrugated-boxes.html",
    "Kraft Paper": "products/kraft-paper.html",
    "Art Card": "products/art-card.html",
    "Gift Boxes": "products/gift-boxes.html",
    "Custom Pouches": "products/custom-pouches.html",
    "Labels & Tags": "products/labels-and-tags.html",
    "Premium Finish": "products/premium-finish.html",
    "Magnetic Snap Box": "products/rigid-boxes/magnetic-snap-box.html",
    "Lid & Base Box": "products/rigid-boxes/lid-and-base-box.html",
    "Drawer Style Box": "products/rigid-boxes/drawer-style-box.html",
    "Book Style Box": "products/rigid-boxes/book-style-box.html",
    "Shoulder & Neck Box": "products/rigid-boxes/shoulder-and-neck-box.html",
    "Collapsible Rigid Box": "products/rigid-boxes/collapsible-rigid-box.html",
    "Circular Rigid Tube": "products/rigid-boxes/circular-rigid-tube.html",
    "Hexagonal Rigid Box": "products/rigid-boxes/hexagonal-rigid-box.html",
    "Custom Rigid Box": "products/rigid-boxes/custom-rigid-box.html",
    "Full-Color Mailer Box": "products/corrugated-boxes/full-color-mailer-box.html",
    "Litho-Laminated Luxury Box": "products/corrugated-boxes/litho-laminated-luxury-box.html",
    "Retail-Ready Packaging (RRP)": "products/corrugated-boxes/retail-ready-packaging-rrp.html",
    "Standard Shipping Box (RSC)": "products/corrugated-boxes/standard-shipping-box-rsc.html",
    "Front Tuck Mailer": "products/corrugated-boxes/front-tuck-mailer.html",
    "Pizza & Food Box": "products/corrugated-boxes/pizza-and-food-box.html",
    "Subscription Box": "products/corrugated-boxes/subscription-box.html",
    "Display Floor Stand": "products/corrugated-boxes/display-floor-stand.html",
    "Counter Display Unit (CDU)": "products/corrugated-boxes/counter-display-unit-cdu.html",
    "Double Wall Heavy Duty": "products/corrugated-boxes/double-wall-heavy-duty.html",
    "Custom Industrial Crate": "products/corrugated-boxes/custom-industrial-crate.html"
};

function openModal(productName) {
    // Redirect to the product page using the canonical links map
    const url = productLinks[productName];
    
    if (url) {
        window.location.href = url;
    } else {
        // Fallback: generate slug if not found in map
        const pageSlug = productName.toLowerCase().trim().replace(/\s+/g, '-').replace(/[&]/g, 'and').replace(/[()]/g, '');
        window.location.href = `products/${encodeURIComponent(pageSlug)}.html`;
    }
}

// Opens the full product page (replace with actual URL logic if needed)
function viewFullProductPage() {
    if (window.currentProductName) {
        if (window.currentProductName === 'Rigid Boxes') {
            window.location.href = '/products/rigid-boxes.html';
        } else {
            window.location.href = `/products/${encodeURIComponent(window.currentProductName.replace(/ /g, '-'))}.html`;
        }
    } else {
...
// Separate Toggle for the "Drawer" Bar
function toggleModalForm() {
    const formDiv = document.getElementById('collapsibleForm');
    const icon = document.getElementById('triggerIcon');
    if (formDiv) {
        formDiv.classList.toggle('open');
        icon.innerText = formDiv.classList.contains('open') ? '−' : '+';
    }
}
...
// Show the general overview entry in the modal sidebar
function showGeneralOverview() {
    const modalTitle = document.getElementById('modalTitle');
    const modalDesc = document.getElementById('modalDesc');
    const modalLong = document.getElementById('modalLongContent');

    const name = window.currentProductName || 'Product Overview';

    if (modalTitle) modalTitle.innerText = name + ' — Overview';
    if (modalDesc) modalDesc.innerText = '';

    // Try to use productData if present
    try {
        const data = (typeof productData !== 'undefined' && productData[name]) ? productData[name] : null;
        if (modalLong) modalLong.innerText = data ? (data.overview || data.desc || '') : 'General product information and overview will appear here.';
    } catch (e) {
        if (modalLong) modalLong.innerText = 'General product information and overview will appear here.';
    }
}
// 1. Navigation for the Wizard
function nextStep(step) {
    ...
    const targetStep = (step === 'Success') ? 'stepSuccess' : 'step' + step;
    const currentEl = document.getElementById(targetStep);
    ...
    // Update the dots (step indicators) - only for steps 1, 2, 3
    if (typeof step === 'number') {
        document.querySelectorAll('.step-dot').forEach((dot, index) => {
            dot.classList.toggle('active', index < step);
        });
    } else {
        // Hide indicators on success
        document.querySelector('.step-indicator').style.display = 'none';
    }
}

// 2. Handle the Final Submission
document.getElementById('quoteWizard').addEventListener('submit', function(e) {
    e.preventDefault();
    
    // Here you would normally add your email/database logic
...
// KINETIC CARDS: Now view-only (click handlers removed)
document.querySelectorAll('.kinetic-card').forEach(card => {
    // These cards are informational only and do not navigate
});

    
    // Here you would normally add your email sending logic (like EmailJS or a backend)
    
    // Transition to success state
    document.querySelectorAll('.form-step').forEach(s => s.classList.remove('active'));
    document.getElementById('stepSuccess').classList.add('active');
    
    // Optional: Hide the step dots since we are finished
    document.querySelector('.step-indicator').style.display = 'none';
;
function buildGlobalUI() {
    ...
    if (navDropdown) {
        navDropdown.innerHTML = Object.entries(catalog).map(([category, subs]) => {
            const hasSubs = subs.length > 0;
            
            return `
                <li class="dropdown-item">
                    <a href="javascript:void(0)" onclick="openModal('${category}')">
                        ${category} ${hasSubs ? '<span class="arrow-right">▶</span>' : ''}
                    </a>
                    ${hasSubs ? `
                        <ul class="submenu">
                            ${subs.map(s => `
                                <li>
                                    <a href="javascript:void(0)" onclick="openModal('${s.replace(/'/g, "\\'")}')">
                                        ${s}
                                    </a>
                                </li>
                            `).join('')}
    ...
    // 2. Kinetic Cards: View-only display (removed clickable behavior)
    document.querySelectorAll('.kinetic-card').forEach(card => {
        // These cards are now informational only - no click handlers
        card.style.cursor = 'default'; // Remove pointer cursor
    });
}

// FINAL STEP: Ensure the function runs when the page loads
document.addEventListener('DOMContentLoaded', buildGlobalUI);
...
function changeSubProduct(imagePath, title, desc) {
    ...
    setTimeout(() => {
        ...
        // Keep the View Details button visible when switching sub-products
        const btn = document.getElementById('viewFullPageBtn');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (mobileMenuToggle) {
    mobileMenuToggle.addEventListener('click', function() {
      ...
      mobileMenuBackdrop.style.display = mobileMenu.classList.contains('active') ? 'block' : 'none';
